...
const Head: React.FC<HeadRFCProps> = ({
  children,
  ...props
}): any | null => {
  ...
  const { 
    ...
    pageDescription, pageKeywords, pageAuthor, pageType, pageThumbnail,
    themeColor, locales, language, languageDirection, charSet, favicons, styles, appClassNames,
    siteName, siteLocale, twitterCard, twitterUsername, twitterAuthor
   } = props;
  ...
  // Meta
  const _direction: string = languageDirection || snippets.constants.DEFAULT_DIRECTION;

  const _seperator: string = titleSeparator || snippets.constants.DEFAULT_SEPERATOR;

  const _description: string = pageDescription || snippets.constants.DEFAULT_DESCRIPTION;
  ...
  const _thumbnail: string = pageThumbnail || snippets.constants.DEFAULT_THUMBNAIL;

  const _twitter_username: string = twitterUsername || snippets.constants.TWITTER_USERNAME;

  const _twitter_author: string = twitterAuthor || snippets.constants.TWITTER_AUTHOR;

  const _twitter_card: string = twitterCard || snippets.constants.DEFAULT_CARD;

  // Locales
  const _locales: Array<IFavicon> = locales || generateLocales(snippets.constants.LOCALES);

  // Favicons
  const _favicons: Array<IFavicon> = favicons || generateFavicons(snippets.constants.FAVICONS);

  //Styles
  const _styles: Array<IStyle> = styles || generateStyles(snippets.constants.STYLES);
  ...
  return (
    <>
      <Helmet>
        ...
        {/* Title */}
        <title>
          {_direction === snippets.constants.LTR ? _title : snippets.constants.SITE_NAME}
          {" "}
          {_seperator}
          {" "}
          {_direction === snippets.constants.RTL ? snippets.constants.SITE_NAME : _title}
        </title>
        ...
        <meta name="theme-color" content={_themeColor} media="(prefers-color-scheme: dark)" />
        <meta name="theme-color" content={_themeColor} media="(prefers-color-scheme: light)" />
        ...
        {_locales?.map((locale:any, localeIndex:number) => (
          <link key={localeIndex} rel="alternate" href={`${locationHostURL}/${locale}`} lang={locale} />
        ))}

        {/* Favicons */}
        {_favicons?.map((favicon: IFavicon, faviconIndex: number) => (
          <link key={faviconIndex} rel={favicon.rel} type={favicon.type} sizes={favicon.sizes} href={`/assets/images/favicons/${favicon.href}`} />
        ))}
        ...
        {_styles?.map((link: IStyle, linkIndex: number) => (
          <link key={linkIndex} rel={link.rel} type={link.type} href={`/assets/styles/${link.href}`} />
        ))}
