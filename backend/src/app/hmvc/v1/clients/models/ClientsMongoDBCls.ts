...
export class ClientsMongoDBCls extends MongoDBCollectionCls {

  // Clients model object
  private ClientsMongoDBModel: any;

  // Clients schema object
  ...
  constructor() {
    ...
    this.clientsSchema = new Schema<IClients>({
		...
		client_device_pub_key: {
         type: String,
         required: false,
         trim: true
		},
		client_device_secret: {
         type: String,
         required: false,
         trim: true
		},
		client_device_updated_at: {
         type: String,
         required: false,
         trim: true
		},
    ...
    // Define clients auto increment key

    this.clientsSchema.plugin(autoIncrement, { model: this.modelName, field: '_id' });

    this.ClientsMongoDBModel = model<IClients>(this.modelName, this.clientsSchema);

    // Create a change stream cursor that listens for changes to the collection
    this.ClientsMongoDBModel.watch();


    // Explicitly create the collection before using it
    // so the collection is capped.
    this.ClientsMongoDBModel.createCollection();

    this.setupModel(this.ClientsMongoDBModel);
