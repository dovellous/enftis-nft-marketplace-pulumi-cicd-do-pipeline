...
export class BonusesMongoDBCls extends MongoDBCollectionCls {

  // Bonuses model object
  private BonusesMongoDBModel: any;

  // Bonuses schema object
  private bonusesSchema: any;
  ...
  constructor() {
    ...
    // Init bonuses schema
    this.bonusesSchema = new Schema<IBonuses>({
      	_id: {
         type: String,
         required: false,
         trim: true
    ...
    // Define bonuses auto increment key

    this.bonusesSchema.plugin(autoIncrement, { model: this.modelName, field: '_id' });

    this.BonusesMongoDBModel = model<IBonuses>(this.modelName, this.bonusesSchema);

    // Create a change stream cursor that listens for changes to the collection
    this.BonusesMongoDBModel.watch();


    // Explicitly create the collection before using it
    // so the collection is capped.
    this.BonusesMongoDBModel.createCollection();

    this.setupModel(this.BonusesMongoDBModel);
