...
export class BidsMongoDBCls extends MongoDBCollectionCls {

  // Bids model object
  private BidsMongoDBModel: any;

  // Bids schema object
  private bidsSchema: any;
  ...
  constructor() {
    ...
    // Init bids schema
    this.bidsSchema = new Schema<IBids>({
      	_id: {
         type: String,
         required: false,
         trim: true
    ...
    // Define bids auto increment key

    this.bidsSchema.plugin(autoIncrement, { model: this.modelName, field: '_id' });

    this.BidsMongoDBModel = model<IBids>(this.modelName, this.bidsSchema);

    // Create a change stream cursor that listens for changes to the collection
    this.BidsMongoDBModel.watch();


    // Explicitly create the collection before using it
    // so the collection is capped.
    this.BidsMongoDBModel.createCollection();

    this.setupModel(this.BidsMongoDBModel);
