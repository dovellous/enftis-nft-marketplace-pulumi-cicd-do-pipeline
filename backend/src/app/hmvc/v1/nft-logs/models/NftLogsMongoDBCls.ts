...
export class NftLogsMongoDBCls extends MongoDBCollectionCls {

  // NftLogs model object
  private NftLogsMongoDBModel: any;

  // NftLogs schema object
  private nftLogsSchema: any;
  ...
  constructor() {
    ...
    // Init nftLogs schema
    this.nftLogsSchema = new Schema<INftLogs>({
      	_id: {
         type: String,
         required: false,
         trim: true
    ...
    // Define nftLogs auto increment key

    this.nftLogsSchema.plugin(autoIncrement, { model: this.modelName, field: '_id' });

    this.NftLogsMongoDBModel = model<INftLogs>(this.modelName, this.nftLogsSchema);

    // Create a change stream cursor that listens for changes to the collection
    this.NftLogsMongoDBModel.watch();


    // Explicitly create the collection before using it
    // so the collection is capped.
    this.NftLogsMongoDBModel.createCollection();

    this.setupModel(this.NftLogsMongoDBModel);
