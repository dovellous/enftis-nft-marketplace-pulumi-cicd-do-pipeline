...
export class MarketItemsMongoDBCls extends MongoDBCollectionCls {

  // MarketItems model object
  private MarketItemsMongoDBModel: any;

  // MarketItems schema object
  private marketItemsSchema: any;
  ...
  constructor() {
    ...
    // Init marketItems schema
    this.marketItemsSchema = new Schema<IMarketItems>({
      	_id: {
         type: String,
         required: false,
         trim: true
		...
		market_item_created_at: {
         type: String,
         required: false,
         trim: true
		},
		market_item_featured: {
         type: Boolean,
         required: false,
         trim: true
		},
    ...
    // Define marketItems auto increment key

    this.marketItemsSchema.plugin(autoIncrement, { model: this.modelName, field: '_id' });

    this.MarketItemsMongoDBModel = model<IMarketItems>(this.modelName, this.marketItemsSchema);

    // Create a change stream cursor that listens for changes to the collection
    this.MarketItemsMongoDBModel.watch();


    // Explicitly create the collection before using it
    // so the collection is capped.
    this.MarketItemsMongoDBModel.createCollection();

    this.setupModel(this.MarketItemsMongoDBModel);
...
// End Class: MarketItemsMongoDBCls