...
export class NftsMongoDBCls extends MongoDBCollectionCls {

  // Nfts model object
  private NftsMongoDBModel: any;

  // Nfts schema object
  private nftsSchema: any;
  ...
  constructor() {
    ...
    // Init nfts schema
    this.nftsSchema = new Schema<INfts>({
      	_id: {
         type: String,
         required: false,
         trim: true
    ...
    // Define nfts auto increment key

    this.nftsSchema.plugin(autoIncrement, { model: this.modelName, field: '_id' });

    this.NftsMongoDBModel = model<INfts>(this.modelName, this.nftsSchema);

    // Create a change stream cursor that listens for changes to the collection
    this.NftsMongoDBModel.watch();


    // Explicitly create the collection before using it
    // so the collection is capped.
    this.NftsMongoDBModel.createCollection();

    this.setupModel(this.NftsMongoDBModel);
