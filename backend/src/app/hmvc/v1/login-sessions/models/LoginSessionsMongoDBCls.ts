...
export class LoginSessionsMongoDBCls extends MongoDBCollectionCls {

  // LoginSessions model object
  private LoginSessionsMongoDBModel: any;

  // LoginSessions schema object
  private loginSessionsSchema: any;
  ...
  constructor() {
    ...
    // Init model name
    this.modelName = 'LoginSessions';

    // Define the Interface
    interface ILoginSessions {
      	_id : string;
		client_id : number;
		fid : string;
    ...
    // Init loginSessions schema
    this.loginSessionsSchema = new Schema<ILoginSessions>({
      	_id: {
         type: String,
         required: false,
         trim: true
    ...
    // Define loginSessions auto increment key

    this.loginSessionsSchema.plugin(autoIncrement, { model: this.modelName, field: '_id' });

    this.LoginSessionsMongoDBModel = model<ILoginSessions>(this.modelName, this.loginSessionsSchema);

    // Create a change stream cursor that listens for changes to the collection
    this.LoginSessionsMongoDBModel.watch();


    // Explicitly create the collection before using it
    // so the collection is capped.
    this.LoginSessionsMongoDBModel.createCollection();

    this.setupModel(this.LoginSessionsMongoDBModel);
...
// End Class: LoginSessionsMongoDBCls