...
export class AuthorsMongoDBCls extends MongoDBCollectionCls {

  // Authors model object
  private AuthorsMongoDBModel: any;

  // Authors schema object
  private authorsSchema: any;
  ...
  constructor() {
    ...
    // Init authors schema
    this.authorsSchema = new Schema<IAuthors>({
      	_id: {
         type: String,
         required: false,
         trim: true
    ...
    // Define authors auto increment key

    this.authorsSchema.plugin(autoIncrement, { model: this.modelName, field: '_id' });

    this.AuthorsMongoDBModel = model<IAuthors>(this.modelName, this.authorsSchema);

    // Create a change stream cursor that listens for changes to the collection
    this.AuthorsMongoDBModel.watch();


    // Explicitly create the collection before using it
    // so the collection is capped.
    this.AuthorsMongoDBModel.createCollection();

    this.setupModel(this.AuthorsMongoDBModel);
