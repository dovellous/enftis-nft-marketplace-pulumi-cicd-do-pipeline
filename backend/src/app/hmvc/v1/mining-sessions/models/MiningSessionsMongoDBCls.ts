...
export class MiningSessionsMongoDBCls extends MongoDBCollectionCls {

  // MiningSessions model object
  private MiningSessionsMongoDBModel: any;

  // MiningSessions schema object
  private miningSessionsSchema: any;
  ...
  constructor() {
    ...
    // Init model name
    this.modelName = 'MiningSessions';

    // Define the Interface
    interface IMiningSessions {
      	_id : string;
		client_id : number;
		fid : string;
    ...
    // Init miningSessions schema
    this.miningSessionsSchema = new Schema<IMiningSessions>({
      	_id: {
         type: String,
         required: false,
         trim: true
    ...
    // Define miningSessions auto increment key

    this.miningSessionsSchema.plugin(autoIncrement, { model: this.modelName, field: '_id' });

    this.MiningSessionsMongoDBModel = model<IMiningSessions>(this.modelName, this.miningSessionsSchema);

    // Create a change stream cursor that listens for changes to the collection
    this.MiningSessionsMongoDBModel.watch();


    // Explicitly create the collection before using it
    // so the collection is capped.
    this.MiningSessionsMongoDBModel.createCollection();

    this.setupModel(this.MiningSessionsMongoDBModel);
...
// End Class: MiningSessionsMongoDBCls