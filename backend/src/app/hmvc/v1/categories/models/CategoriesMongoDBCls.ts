...
export class CategoriesMongoDBCls extends MongoDBCollectionCls {

  // Categories model object
  private CategoriesMongoDBModel: any;

  // Categories schema object
  private categoriesSchema: any;
  ...
  constructor() {
    ...
    // Init categories schema
    this.categoriesSchema = new Schema<ICategories>({
      	_id: {
         type: String,
         required: false,
         trim: true
    ...
    // Define categories auto increment key

    this.categoriesSchema.plugin(autoIncrement, { model: this.modelName, field: '_id' });

    this.CategoriesMongoDBModel = model<ICategories>(this.modelName, this.categoriesSchema);

    // Create a change stream cursor that listens for changes to the collection
    this.CategoriesMongoDBModel.watch();


    // Explicitly create the collection before using it
    // so the collection is capped.
    this.CategoriesMongoDBModel.createCollection();

    this.setupModel(this.CategoriesMongoDBModel);
...
// End Class: CategoriesMongoDBCls